...
export default function RankedBarChart({
  ...
  const liveScores = useMemo(() =>
    computeSportScoreList(athletes, attributeMeta, weights),
  [athletes, attributeMeta, weights]);

  const data = athletes
    .map(a => ({ ...a, score: Math.round((liveScores[a.id]?.score || 0) * 100) }))
            ...
            <BarChart data={data} layout="vertical" margin={{ left: 20, right: 20, top: 4, bottom: 4 }}>
              ...
              <Bar dataKey="score" radius={[0, 4, 4, 0]}>
                {data.map(entry => (
                  <Cell
                    key={entry.id}
                    fill={selectedAthletes.includes(entry.id) ? '#F59E0B' : color}
                    fillOpacity={selectedAthletes.includes(entry.id) ? 1 : 0.7}
                    stroke={selectedAthletes.includes(entry.id) ? '#FBBF24' : 'none'}
                    strokeWidth={selectedAthletes.includes(entry.id) ? 2 : 0}
                  />
                ))}
              </Bar>
            </BarChart>
