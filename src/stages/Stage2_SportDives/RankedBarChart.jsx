...
export default function RankedBarChart({ athletes, sportScores, selectedAthletes, weights, color, selectedTags }) {
  ...
  const liveScores = useMemo(() =>
    computeSportScoreList(safeAthletes, safeWeights),
  [safeAthletes, safeWeights]);
  ...
  const CustomTooltip = ({ active, payload }) => {
    ...
    return (
      <div style={{
        ...
        <p style={{ color, margin: '0 0 8px' }}>Score: {athlete?.score}</p>
        {activeMetrics.map(m => {
          const rawValue = breakdown[m.key] || 0;
          const w = Math.round((safeWeights[m.key] || 0) * 100);
          return (
