...
export default function Podium({ top3, selectedTags = [] }) {
  ...
  const radarAxes = selectedTags;

  const radarData = radarAxes.map((tag) => {
    const point = { axis: tag };
    top3.forEach((a, i) => {
      point[`p${i}`] = Math.round(
        keywordScoreFromBreakdown(a.breakdown || {}, tag) * 100,
      );
    });
    return point;
  });
    ...
    <div>
      ...
      {radarAxes.length > 0 && (
        ...
        >
          ...
          >
            Top 3 — radar breakdown (selected keywords)
          </p>
          <div
            style={{
              display: "flex",
              ...
              >
                <div
                  style={{
                    ...
                    height: 10,
                    borderRadius: "50%",
                    backgroundColor: PALETTE_RADAR[i],
                  }}
                />
