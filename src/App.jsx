...
export default function App() {
  ...
  const [chartHighlightedAthleteId, setChartHighlightedAthleteId] = useState(null);
  const [leaderboardHoveredAthleteId, setLeaderboardHoveredAthleteId] = useState(null);
  ...
  const leftOffset = showPanels && isDesktopDocked ? (panelOpen ? PANEL_WIDTH : 20) : 0;
  const rightOffset = showPanels && isDesktopDocked ? (leaderboardOpen ? RIGHT_PANEL_WIDTH + 16 : 24) : 0;
  const isSportDive = currentStage === 1;
  const isCrossSport = currentStage === 2;
  const highlightedAthleteId = isCrossSport ? (leaderboardHoveredAthleteId ?? chartHighlightedAthleteId) : null;

  const handleSportDiveLeaderboardClick = useCallback((athleteId) => {
    setSelectedAthletes((prev) => {
      const current = prev[currentSport] || [];
      const exists = current.includes(athleteId);
      const next = exists ? current.filter((id) => id !== athleteId) : [...current, athleteId];
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#0f0f0f', color: '#e5e5e5' }}>
      ...
      {showPanels && (
        <RightVerdictPanel
          ...
          onAthleteClick={isSportDive ? handleSportDiveLeaderboardClick : undefined}
